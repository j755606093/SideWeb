...
/***/ function(module, exports, __webpack_require__) {
	   ...
	   */
			...
			showGetMoney: function showGetMoney() {
				...
				fetch(config.serverUrl + "/api/Member/GetOfflineUsr", {
					...
					if (getData) {
						_this2.moneyIndex++;
						_this2.getMoneyUseData.ReceiveBkge = getData.ReceiveBkge;
						_this2.getMoneyUseData.TotalBkge = getData.TotalBkge;
						_this2.getMoneyUseData.UndoneBkge = getData.UndoneBkge;
						var data = _this2.getMoneyData;
						for (var i = 0; i < getData.Usrs.length; i++) {
							data.push(getData.Usrs[i]);
						}
						_this2.getMoneyData = data;
				...
				fetch(config.serverUrl + "/api/Order/List", {
					...
					if (result.Data) {
						for (var i = 0; i < result.Data.length; i++) {
							_this3["OrderType" + type].OrderList.push(result.Data[i]);
						}
					} else {
						_this3["OrderType" + type].noMoreData = true;
						_this3.toast(result.Message);
					}

					_this3["OrderType" + type].isUse = false; //释放这个函数
					_this3["OrderType" + type].index++;
					_mintUi.Indicator.close();
				}).then(function () {
					_this3.canvas();
				});
	   ...
	   */
			...
			addPassenger: function addPassenger() {
				...
				if (index === 0) {
					fetch(config.serverUrl + "/api/Passenger/Add", {
						...
						if (result.Data) {
							var json = {};
							json.Name = _this4.passengerName;
							json.Mobile = _this4.passengerPhone;
							json.Id = result.Data;
							_this4.passengerPhone = "";
							_this4.passengerName = "";
							_this4.Passenger.push(json);
							_this4.toast("增加成功");
						} else {
							_this4.toast(result.Message);
						}
						_this4.hideChangePassenger();
					});
				} else {
					fetch(config.serverUrl + "/api/Passenger/Modify", {
						...
						if (result.Data) {
							...
							json.Name = _this4.passengerName;
							json.Mobile = _this4.passengerPhone;
							json.Id = _this4.Passenger[_this4.ChaPassengerIndex].Id;
							_this4.Passenger[_this4.ChaPassengerIndex] = json;
							_this4.passengerPhone = "";
							_this4.passengerName = "";
							_this4.toast("修改成功");
						} else {
			...
			trashPassenger: function trashPassenger(index) {
				...
				_mintUi.MessageBox.confirm('确定执行此操作?').then(function (action) {
					fetch(config.serverUrl + "/api/Passenger/Delete/" + _this5.Passenger[index].Id, {
						headers: config.headers
					}).then(checkStatus).then(function (result) {
						return result.json();
					}).then(function (result) {
						if (result.Data) {
							_this5.Passenger.splice(index, 1);
							_this5.toast("删除成功");
						} else {
					...
					if (value === "") {
						...
						fetch(config.serverUrl + "/api/Transport/UpdateUserInfo", {
							...
							if (result.Code === 200) {
								if (index === 0) {
									// 修改昵称
									_this6.UserInfo.Nickname = value;
								}
								if (index === 2) {
									_this6.UserInfo.Mobile = value;
								}
							}
			...
			applyRefund: function applyRefund() {
				...
				this.getRefund().then(function (result) {
					...
					if (result.Data) {
						for (var i = 0; i < result.Data.length; i++) {
							_this7.RefundOrder.RefundList.push(result.Data[i]);
						}
						if (result.Data.length < 10) {
							// 没有更多数据
							_this7.RefundOrder.noMoreData = true;
						}
						_this7.RefundOrder.index++;
					} else {
						_this7.RefundOrder.noMoreData = true;
					}

					_this7.RefundOrder.isUse = false;
				});
			},
			...
			applyGetMoney: function applyGetMoney() {
				...
				if (canGetMoney === 0) {
					...
					this.myModal = true;
					_mintUi.MessageBox.prompt("¥1-¥" + canGetMoney, "请输入提取佣金数额", {
						inputType: 'tel'
					}).then(function (_ref2) {
	...
	document.getElementById("money-lists").addEventListener('scroll', _.throttle(function () {
		...
		var money_last = document.getElementById("money_last").offsetTop - document.getElementById("refund-lists").scrollTop;
