...
/***/ function(module, exports, __webpack_require__) {
	...
	var _mintUi = __webpack_require__(70);

	__webpack_require__(94);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	_vue2.default.use(__webpack_require__(96)); //引用ajax库
	__webpack_require__(97);

	var _ = __webpack_require__(99);


	_vue2.default.component(_mintUi.TabContainer.name, _mintUi.TabContainer);
	_vue2.default.component(_mintUi.TabContainerItem.name, _mintUi.TabContainerItem);
	_vue2.default.component(_mintUi.Tabbar.name, _mintUi.Tabbar);
	_vue2.default.component(_mintUi.TabItem.name, _mintUi.TabItem);
	_vue2.default.component(_mintUi.Navbar.name, _mintUi.Navbar);
	_vue2.default.component(_mintUi.Checklist.name, _mintUi.Checklist);
	...
	var Vue_Order = new _vue2.default({
		...
		created: function created() {
			...
			// 顺序请求服务器,防止服务器请求过多报错
			this.moreOrderData().then(function (result) {
				_this.ready = true;
				_this.moreOrderData1();
		...
		methods: {
			...
			moreOrderData: function moreOrderData() {
				...
				return new _promise2.default(function (resolve, reject) {
					if (_this2.noMoreData || _this2.isUse) {
						if (!empty) {
							resolve();
						}
						_this2.index = 1;
					}
					_this2.isUse = true; //锁住这个函数
					_this2.loading();
			...
			moreOrderData1: function moreOrderData1() {
				...
				fetch(config.serverUrl + "/api/Order/List", {
					...
					if (result.Data) {
						...
						if (empty) {
							_this3.OrderList1 = result.Data;
						} else {
							for (var i = 0; i < result.Data.length; i++) {
								_this3.OrderList1.push(result.Data[i]);
							}
						}
						_this3.noDataShow1 = false; //显示订单
					} else {
						_this3.noMoreData1 = true;
						if (_this3.index1 !== 1) {
			...
			openOrder: function openOrder(index) {
				...
				if (this.selected === 1) {
					...
					var id = this.OrderList1[index].Id;
			...
			getOrderInfo: function getOrderInfo(id) {
				...
				return fetch(config.serverUrl + "/api/Order/Get", {
					...
					_this4.OrderDetail = result.Data;
					_this4.passenger = []; //显示乘客状态
					_this4.optionsPassenger = []; //提供申请退款选择的用户名
					for (var i = 0; i < _this4.OrderDetail.Passengers.length; i++) {
						var item = _this4.OrderDetail.Passengers[i];
						if (item.Status === -3) {
							item.Name = item.Name + "(已退款)";
							_this4.passenger.push(item);
						...
						if (item.Status === 1) {
							_this4.passenger.push(item);
							_this4.optionsPassenger.push({ Name: item.Name, Price: item.Price, DId: item.DId, active: false }); //提供申请退款选择的用户名
						}
						if (item.Status === 2) {
					...
					if (_this4.selected === 1) {
						// 只有未支付的情况下才倒计时
						_this4.CountDown();
					}
					/** 判断订单状态来显示二维码 */
					if (_this4.OrderDetail.IsPay === 1 && (_this4.OrderDetail.Status === 2 || _this4.OrderDetail.Status === 4)) {
						// 显示验证码
						_this4.showCode = true;
					} else {
						// 其它情况,不显示验证码
						_this4.showCode = false;
					}

					//判断订单是否支付,选择页面显示
					if (_this4.OrderDetail.IsPay === 1) {
						_this4.selected = 2;
					}
				}).catch(function (error) {
					console.log(error);
					...
					window.WeixinJSBridge.invoke("getBrandWCPayRequest", paydata, function (r) {
						if (r.err_msg === "get_brand_wcpay_request:ok") {
							...
							_mintUi.MessageBox.alert('支付成功').then(function (action) {
								...
								_this5.goback();
								_this5.moreOrderData(true);
								_this5.moreOrderData1(true);
							});
						}
					});
				});
			...
			CountDown: function CountDown() {
				...
				this.countdown = setInterval(function () {
					if (_this6.storeCountTime <= 0) {
						...
						_this6.countdownTime = "已经失效";
						return;
					}
					var minth = parseInt(_this6.storeCountTime / 60);
					var second = parseInt(_this6.storeCountTime % 60);
					if (minth < 10) {
						_this6.countdownTime = "0" + minth + ":";
					} else {
						_this6.countdownTime = minth + ":";
					}

					if (second < 10) {
						_this6.countdownTime = _this6.countdownTime + "0" + second;
					} else {
						_this6.countdownTime = _this6.countdownTime + second;
					}
			...
			checkSelectPassenger: function checkSelectPassenger() {
				...
				if (this.selectPassenger.length === 0) {
					_mintUi.MessageBox.alert("你未选择退款乘客").then(function (result) {
			...
			refund: function refund() {
				...
				// /api/Order/Refund
				var id = this.OrderDetail.Id;
				var len = this.OrderDetail.Passengers.length;
				var DIds = [];
				if (len === 1) {
					//就全部退掉
					for (var i = 0; i < this.OrderDetail.Passengers.length; i++) {
					...
					//退掉用户选中的
					for (var _i = 0; _i < this.selectPassenger.length; _i++) {
						DIds.push(this.selectPassenger[_i]);
					}
				}
					...
					fetch(config.serverUrl + "/api/Order/Refund", {
						...
						if (result.Data) {
							...
							_mintUi.MessageBox.alert('申请退款已提交').then(function (result) {
								_this10.myModal = false;
								_this10.goback(); //返回上一层
							});
							// 重新加载数据
...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(92), (function() { return this; }())))
