		...
		methods: {
			...
			CountDown: function CountDown() {
				...
				this.countdown = setInterval(function () {
					...
					var second = parseInt(_this5.storeCountTime % 60);
					if (minth < 10) {
						_this5.countdownTime = "0" + minth + ":";
					} else {
						_this5.countdownTime = minth + ":";
					}

					if (second < 10) {
						_this5.countdownTime = _this5.countdownTime + "0" + second;
					} else {
						_this5.countdownTime = _this5.countdownTime + second;
					}
					_this5.storeCountTime--;
				}, 1000);
			},
				...
				_mintUi.MessageBox.confirm('确定取消订单?').then(function (action) {
					fetch(config.serverUrl + "/api/Order/Cancel", {
						...
						_mintUi.MessageBox.alert('取消订单成功').then(function (res) {
							_this6.myModal = false;
						});
						_this6.goback();
						if (!_this6.OrderList1) {
				...
				this.inputRefund().then(function (result) {
					fetch(config.serverUrl + "/api/Order/Refund", {
						...
						if (result.Data) {
							...
							(0, _mintUi.MessageBox)('提示', result.Message);
						}
					});
				}).catch(function (error) {
			...
			selectRefund: function selectRefund(index) {
				...
				this.optionsPassenger.map(function (item, i) {
					if (item.active) {
						...
						_this10.refunndMoney += money;
						_this10.selectPassenger.push(item.DId);
					}
				});
